...
export default function TransferPage() {
    ...
    const handleTransfer = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(`Sent ${amount} points to ${email}!`);
    ...
    return (
        <div className="p-4 max-w-md mx-auto space-y-6">
            <h1 className="text-2xl font-black text-wawag-purple text-center">Send Points 💸</h1>

            <Card className="p-6 space-y-6">
                <form onSubmit={handleTransfer} className="space-y-4">
                    <div>
                        <label className="block text-sm font-bold text-gray-600 mb-1">Receiver Email</label>
                        <input
                            type="email"
                            required
                            value={email}
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-600 mb-1">Amount (Min 10)</label>
                        <input
                            type="number"
                            min="10"
                            required
