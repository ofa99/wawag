...
export async function verifyIdToken(token) {
    try {
        ...
        return { ...user, uid: user.localId }; // { localId, uid, email, ... }
    } catch (error) {
...
export async function runTransaction(token, updateFunction) {
    ...
    const transaction = {
        get: async (collectionName, docId) => {
            const res = await fetch(`${FIRESTORE_URL}/${collectionName}/${docId}?transaction=${encodeURIComponent(transactionId)}`, {
                headers: { "Authorization": `Bearer ${token}` }
            });
            if (res.status === 404) return { exists: false, data: null, id: docId };
            if (!res.ok) {
                const errText = await res.text();
                console.error(`Transaction Get Error (${collectionName}/${docId}):`, res.status, errText);
                throw new Error("Failed to get document in transaction: " + errText);
            }
